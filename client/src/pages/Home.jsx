...
function Home() {
  ...
  // Kumpulkan semua kategori unik dari semua produk
  ...
  const tagColorMap = {
    ...
    VIP: 'bg-pink-400',
    FREEFIRE: 'bg-orange-500',
    MLBB: 'bg-blue-500',
    RESELLER: 'bg-violet-400',
  };
  const getBadgeColor = (tag) => tagColorMap[tag.toUpperCase()] || 'bg-neo-green';

  const features = [
    { icon: <DollarSign size={32} strokeWidth={2.5} />, color: 'bg-neo-green', title: 'Harga Termurah', desc: 'Harga murah & kompetitif' },
    { icon: <Zap size={32} strokeWidth={2.5} />, color: 'bg-yellow-300', title: 'Proses Kilat', desc: 'Pengerjaan cepat' },
    { icon: <Lock size={32} strokeWidth={2.5} />, color: 'bg-sky-300', title: 'Aman', desc: 'Trusted seller & aman' },
    { icon: <Clock size={32} strokeWidth={2.5} />, color: 'bg-purple-300', title: 'Open 24/7', desc: 'Siap melayani kapan saja' },
  ];
    ...
    <div className="p-4 md:p-8 max-w-7xl mx-auto min-h-screen flex flex-col gap-12 pt-12">
        ...
        <div className="flex flex-wrap gap-3 animate-fade-in-up">
          {allCategories.map((cat) => (
            <button
              ...
              className={`border-4 border-neo-dark px-5 py-2 font-black uppercase text-sm tracking-wider transition-all duration-150
                ${activeFilter === cat
                  ? 'bg-neo-dark text-white shadow-none translate-x-0.5 translate-y-0.5'
                  : 'bg-white hover:-translate-y-1 hover:shadow-[4px_4px_0px_0px_#1e293b] shadow-[4px_4px_0px_0px_#1e293b]'
                }`}
            >
              {cat}
            </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {filteredProducts.map(([id, product]) => {
          // Ambil tag pertama untuk badge pojok
          ...
          return (
            ...
            >
              {/* Area Gambar + Badge */}
              <div className="h-48 border-4 border-neo-dark bg-rose-100 flex items-center justify-center overflow-hidden relative font-black uppercase text-4xl">
                {product.image ? (
                  <>
                    <div
                    ...
                    <img
                      src={product.image}
                      alt={product.name}
                      className="relative z-10 w-full h-full object-contain p-2 drop-shadow-[4px_4px_0_rgba(30,41,59,1)]"
                    />
                  </>
                ) : (
                ...
                {/* Badge Tag di pojok kanan atas */}
                {firstTag && (
                  <span className={`absolute top-0 right-0 z-20 ${getBadgeColor(firstTag)} border-l-4 border-b-4 border-neo-dark px-3 py-1 text-xs font-black uppercase text-neo-dark tracking-wider`}>
                    {firstTag}
                  </span>
                )}
              </div>
              ...
              <div className="flex flex-col gap-1">
                ...
                {/* Semua tags kecil di bawah deskripsi */}
                {product.tags && (
                  <div className="flex flex-wrap gap-1 mt-1">
                    {product.tags.split(',').map((t, i) => (
                      <span
                        key={i}
                        className={`${getBadgeColor(t.trim())} border-2 border-neo-dark px-2 py-0.5 text-[10px] font-black uppercase`}
                      >
                        {t.trim()}
                      </span>
                    ))}
      ...
      {filteredProducts.length === 0 && (
        <div className="neo-card text-center py-24 animate-fade-in-up flex flex-col items-center justify-center max-w-2xl mx-auto w-full bg-white text-neo-dark">
          ...
          <h2 className="text-4xl font-black uppercase mb-2">
            {activeFilter === 'ALL' ? 'Wow, Kosong!' : `Tidak ada produk "${activeFilter}"`}
          </h2>
          <p className="font-bold opacity-75">
            {activeFilter === 'ALL' ? 'Toko ini masih belum punya produk buat dijual.' : 'Coba pilih kategori lain.'}
          </p>
        </div>
      )}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 animate-fade-in-up">
        {features.map((f, i) => (
          <div key={i} className="neo-card bg-white flex flex-col items-center text-center gap-3 py-6">
      ...
      <div className="neo-card bg-white animate-fade-in-up">
        ...
        <div className="flex flex-wrap justify-center items-center gap-4">
          ...
          <div className="border-4 border-neo-dark px-5 py-3 shadow-[4px_4px_0px_0px_#1e293b] bg-[#108EE9] text-white font-black text-lg tracking-wider">DANA</div>
          <div className="border-4 border-neo-dark px-5 py-3 shadow-[4px_4px_0px_0px_#1e293b] bg-[#00AED6] text-white font-black text-lg tracking-wider">GoPay</div>
          <div className="border-4 border-neo-dark px-5 py-3 shadow-[4px_4px_0px_0px_#1e293b] bg-[#4C3494] text-white font-black text-lg tracking-wider">OVO</div>
          <div className="border-4 border-neo-dark px-5 py-3 shadow-[4px_4px_0px_0px_#1e293b] bg-[#EE4D2D] text-white font-black text-lg tracking-wider">ShopeePay</div>
        </div>
      </div>
